        ...
        try:
            ...
            if cart.status == False:
                ...
                result = CartDetail.objects.filter(
                    ...
                    quantity=quantity, date_add=strftime("%d-%m-%y", gmtime()))
                if result == 0:
                    CartDetail(quantity=quantity, date_add=strftime("%d-%m-%y", gmtime()),
                               product_id=Product.objects.get(id=id),
                               cart_id=Cart.objects.get(user_id=User.objects.get(id=request.session['id']))).save()
            ...
            CartDetail(quantity=quantity, date_add=strftime("%d-%m-%y", gmtime()),
                       product_id=Product.objects.get(id=id),
                       cart_id=Cart.objects.get(user_id=request.session['id'], status=True)).save()
...
#
# def checkCartDetails(cartid, id):
#     cnt = 0
#     cart_details = CartDetail.objects.filter(cart_id=cartid)
#     if cart_details:
#         print(cart_details)
#         for cartdtl in cart_details:
#             if cartdtl.product_id == id:
#                 cnt = cnt + 1
#         if cnt > 0:
#             return True
#         else:
...
def shoping_cart(request):
    if checkSessionVars(request):
        return render(request, 'userpanel/shoping-cart.html')
    return HttpResponseRedirect(reverse('login'))
