...
def product(request):
    if checkSessionVars(request):
        if request.POST.get('cat_id') is None:
            category = Category.objects.all()
            product = Product.objects.filter(cat_id=category[0].id)
        else:
            c_id = request.POST.get('cat_id')
            product = Product.objects.filter(cat_id=c_id)
            category = Category.objects.all()
        return render(request, 'userpanel/product.html', {'products': product, 'categories': category})
    return HttpResponseRedirect(reverse('login'))
